...
RSpec.describe User, type: :model do
  ...
  describe 'Validations' do
    ...
    # should be first_name and last_name
    it 'should validate name field' do
      @user = User.new(email: 'spiderman@test.com', password: 'spiderWeb', password_confirmation: 'spiderWeb')
      #expect(@user).not_to be_valid #can use just this
      @user.valid?
    ...
    it 'should validate email field' do
      @user = User.new(name: 'Gwen Stacy', password: 'spiderWeb', password_confirmation: 'spiderWeb')
    ...
    it 'should check if user email is unique' do
      @user1 = User.new(name: 'Gwen Stacy', email: 'spiderman@test.com', password: 'spiderWeb', password_confirmation: 'spiderWeb')
      @user1.save
      @user2 = User.new(name: 'Peter Parker ', email: 'SPIDERMAN@test.com', password: 'spiderWeb', password_confirmation: 'spiderWeb')
      @user2.valid?
      expect(@user2.errors[:email]).to include("has already been taken")
    end

    it 'should validate password field' do
      @user = User.new(name: 'Gwen Stacy', email: 'spiderman@test.com')
      #expect(@user).not_to be_valid
      @user.valid?
    ...
    it 'should check if password and password confirmation do match' do
      @user = User.new(name: 'Gwen Stacy', email: 'spiderman@test.com', password: 'spiderWeb', password_confirmation: 'spiderwweb')
      @user.valid?
      expect(@user.errors[:password_confirmation]).to include("doesn't match Password")
    end

    it 'should validate/check if the password is too short' do
      @user = User.new(name: 'Gwen Stacy', email: 'spiderman@test.com', password: 'webby', password_confirmation: 'webby')
      @user.valid?
